...
def test_generate_eval_expression(hdf_map):
    ...
    assert len(data) == 5
    assert data.get('Transmission') == pytest.approx(1)
    assert data.get('signal') == pytest.approx(0)  # signal not defined for HdfMap
    assert sum(data.get('count_time', 0)) == pytest.approx(101)
